...
const { runCommand, displayError } = require('./utils');
...
const addFiles = () => {
  return new Promise(async (resolve, reject) => {
    ...
    if (filesAnswers.files) {
      ...
      let topLevel = await runCommand('git rev-parse --show-toplevel');
      if (topLevel.indexOf('fatal') === 0) topLevel = '';

      filesAnswers.files.forEach(file => {
        // Ignore already staged files
        if (stagedFiles.indexOf(file) < 0) {
          stageCommand += ` ${path.join(topLevel.trim(), file)}`;
        }
      });
...
module.exports = { addFiles, getStagedFiles, getChangedFiles };
